                ...
                <form>
                    ...
                    <select wire:model="taskId" wire:click="updateTaskId" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                        ...
                        @foreach ($tasks as $task)
                            <option value="{{ $task->id }}" @if ($task->id == $taskId) selected @endif>
                                {{ $task->name }} | {{ $task->state->label() }} state | {{ $task->priority->label() }} priority</p>
                            </option>
                        @endforeach
                    </select>

                    <x-secondary-button type="reset" @click="$dispatch('close-modal', 'show-tasks-modal')" class="mt-6">Cancel</x-secondary-button>

                    @if (session()->has('pomodoro.taskId') && session()->get('pomodoro.taskId') == $taskId)
                        <x-primary-button type="button" wire:click="removeTask" @click="$dispatch('close-modal', 'show-tasks-modal')">Remove task</x-primary-button>
                    @else
